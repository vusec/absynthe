...
"""
This file is a heavily modified version of  xmlToAssembler.py at https://uops.info/xml.html
-BJG
"""

arch_filter = sys.argv[1]
...
mydata_start:
.align 512
genericretaddr: .quad genericret
lotsofstackspace_lowaddr_addr: .quad  lotsofstackspace_lowaddr
.align 512
keepsp: .quad 8
.align 512
lotsofstackspace_lowaddr: .space 800000
.align 512
lotsofstackspace_highaddr: .quad 8
...
""")
   ...
   for instrNode in root.iter('instruction'):
      ...
      for archNode in instrNode.iter('architecture'):
          if archNode.attrib['name'] == arch_filter: 
              archfound=True
      if archfound == False:
          archmiss+=1
          continue
      ...
      if '{' not in asm or arch_filter != 'HSW':
              readlabel, asmreadlabel = generatecode(lineno, asm, True)
              readlabels[lineno] = readlabel
              readstringlabels[lineno] = asmreadlabel
              writelabel, asmwritelabel = generatecode(lineno, asm, False)
              writelabels[lineno] = writelabel
              writestringlabels[lineno] = asmwritelabel
   if len(readlabels) < 1 or len(writelabels) < 1 or len(readstringlabels) < 1 or len(writestringlabels) < 1:
       raise Exception('no complete data found for arch', arch_filter)
   generate_table('readlabels', readlabels)
   generate_table('writelabels', writelabels)
   generate_table('readstringlabels', readstringlabels)
   generate_table('writestringlabels', writestringlabels)
   sys.stderr.write('cpl skipped: %d arch skipped: %d extension skipped: %s hit: %d\n' % (cplmiss, archmiss, extensionmiss, hit))
